...
candles_full %>%
  ggplot() +
  aes(x = Date, y = DailyAvg, color = Scented, fill = Scented, shape = Scented) +
...
I personally find the separate graphs to be easier to interpret as there is a lot of data on the combined graph and it can be hard to interpret.
...
scented_data$ScentReview <- str_no_scent(scented_data$Review)
scented_data$ScentReview2 <- factor(scented_data$ScentReview , labels = c("scent", "no scent"))
```

```{r}
Scented_NoSmellRev <- scented_data %>%
...
There is a rise in no scent reviews in 2020 compared to 2019.