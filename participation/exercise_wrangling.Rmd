...
```{r}
select(gapminder,c('year','lifeExp','country')) #c makes a list
```
...
# == is for testing logical values, = is for assigning values
```
...
```{r}
gapminder %>%
  filter(country %in% c("Brazil","China") & pop > 100000000) #%in% is like == but it checks all the values in the vector; == will alternate back and forth
```
